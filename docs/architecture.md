...
The shipped backend lives in `src/viewy/backend/wv/` and wraps the pinned
`vendor/webview` source.
...
in `backend/wv/ffi.nim` rather than generated by c2nim, futhark, or an external
binding package. This keeps dependencies low and limits the exposed native
surface to the functions viewy actually uses.

The backend compiles `vendor/webview/webview.cc` through `backend/wv/build.nim`.
