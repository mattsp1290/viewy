## Lite backend implementation backed by webview/webview.
...
type
  LiteBackendError* = object of CatchableError
    ## Raised when the lite webview backend returns an error status.

  WvBackendError* = LiteBackendError
    ## Deprecated compatibility alias for code that caught the old error type.
...
proc toState(h: BackendHandle): WvState =
  ...
  for state in liveStates:
    if state.shared == shared:
      return state
  raise newException(LiteBackendError, "viewy backend handle is not live")
...
proc expectOk(op: string; err: WebviewError) =
  if err != wvOk:
    raise newException(LiteBackendError, &"{op} failed: {err}")
...
proc requireOpen(shared: ptr SharedState; op: string) =
  if shared.closed or shared.webview == nil:
    raise newException(LiteBackendError, op & " failed: backend is closed")
...
proc create(debug: bool): BackendHandle =
  ...
  if webview == nil:
    raise newException(LiteBackendError, "webview_create failed")

  let shared = cast[ptr SharedState](allocShared0(sizeof(SharedState)))
  if shared == nil:
    discard webviewDestroy(webview)
    raise newException(LiteBackendError, "webview_create failed: out of memory")
  shared[] = SharedState(
    webview: webview,
...
proc dispatch(h: BackendHandle; fn: DispatchProc) {.gcsafe.} =
  ...
  if getThreadId() != state.shared.mainThreadId:
    raise newException(LiteBackendError,
  ...
  if payload == nil:
    state.dispatches[slotIndex] = nil
    raise newException(LiteBackendError, "webview_dispatch failed: out of memory")
...
proc bindFn(h: BackendHandle; name: string; cb: BindCallback) =
  ...
  if state.hasBinding(name):
    raise newException(LiteBackendError,
        "webview_bind failed: duplicate binding " & name)
